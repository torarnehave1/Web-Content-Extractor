...
export const useUserStore = defineStore('user', {
  ...
  actions: {
    ...
    setUser(user) {
      ...
      localStorage.setItem('wcx_user', JSON.stringify({
        email: user.email,
        role: user.role,
    ...
    logout() {
      ...
      this.loggedIn = false
      localStorage.removeItem('wcx_user')
      sessionStorage.removeItem('wcx_session_verified')
      this.clearAuthCookie()
    },

    loadFromStorage() {
      const stored = localStorage.getItem('wcx_user')
      if (stored) {
        try {
